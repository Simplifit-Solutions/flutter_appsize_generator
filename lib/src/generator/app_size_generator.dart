...
class AppSizeGenerator {
  ...
  Future<void> build() async {
    final appSizeClass = ClassBuilderFactory(className: 'AppSize');
    for (int step = rasterSize; step <= maxSize; step += rasterSize) {
      appSizeClass.addConstFields('appSize$step', 'double', step);
    }

    final filepath =path.join( path.current,'lib','generated','app_size.dart');
